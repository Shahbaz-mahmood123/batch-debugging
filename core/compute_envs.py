       ...
       # print(ce['computeEnv']['config']['headJobCpus'])
        ...
        if ce: 
            if head_job_memory_mb is None:
                recommendations["Advanced option - Head Job Memory"]= "For scaleability we recommend increasing the head job memory to a sensible value. Generally we recommend 16GB" 
            if head_job_cpus is None:
                recommendations["Advanced option - Head Job CPUs"] = "For scaleability we recommend increasing the head jobs CPU to 8 cpus"         
